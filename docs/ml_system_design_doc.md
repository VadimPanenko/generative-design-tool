...
**Источники данных:**
- Корпоративный бренд-бук (8 цветов в HEX формате)
- Библиотека 3D-иконок (~200 PNG/SVG изображений)
- Тестовые промпты (50 примеров с категориями и сложностью)
- Результаты генерации (PNG изображения, накапливаются)
- Обратная связь (оценки 1-5, 100+ примеров)

**Характеристики промптов (EDA анализ):**
- Средняя длина: 29 символов, 4.4 слова
- Категории: UI (30%), техника (24%), бизнес (20%), e-commerce (14%)
- Сложность: 50% простые, 40% средние, 10% сложные
- Только 50% содержат "3D", 8% указывают стиль явно

### Выявленные проблемы с данными

**Объем:** Недостаточно данных для fine-tuning (50 промптов vs 1000+ требуемых)

**Качество:** Промпты слишком короткие, отсутствие стандартизации, субъективные оценки

**Разметка:** Нет ground truth изображений, оценки зависят от контекста использования

### Риски и проблемы, выявленные на этапе EDA

1. **Короткие промпты** - риск галлюцинаций модели (высокая вероятность)
2. **Отсутствие стилистических указаний** - риск несоответствия бренд-буку (высокая вероятность)
3. **Недостаточный объем данных** - ограничивает развитие модели (средняя вероятность)
4. **Субъективность оценок** - среднее соответствие бренд-буку 3.73/5

### Описание процесса генерации данных

**Источники поступления:**
1. Корпоративные гайдлайны: Figma → HEX коды, обновления 1-2 раза/год
2. Промпты: Веб-интерфейс → база данных, real-time сбор
3. Результаты: FLUX.1-dev → PNG с привязкой к промпту
4. Обратная связь: UI оценки → JSON, после каждой генерации

### Решение проблемы недостатка данных

**Стратегии:**
1. **Промпт-инженеринг** вместо fine-tuning на текущем этапе
2. **Накопление в production** (цель: 1000+ примеров за 3 месяца пилота)
3. **Синтетическая генерация** вариаций промптов

### Конфиденциальность данных

**Меры:** Локальное хранение, анонимизация промптов, шифрование результатов, ролевой доступ

### Необходимый результат этапа

- Датасет корпоративных цветов и промптов
- Pipeline сбора обратной связи
- Система хранения данных
- Процедуры backup и конфиденциальности
...
**Основные метрики:**
- **CLIP Score** (>0.75): семантическое соответствие промпта изображению
- **FID** (<50): качество генерации vs реальные изображения
- **Соответствие бренд-буку** (>3.5/5): экспертная оценка
- **Время генерации** (<4 мин): производительность
- **Success Rate** (>95%): процент успешных генераций

**Функции потерь:**
- Flow-matching loss (FLUX.1-dev встроенная)
- MSE между CLIP эмбеддингами
- Cosine similarity для стилистической оценки

### Описание схемы ML-валидации

**Особенности данных:**
- Небалансированные категории промптов
- Субъективные оценки качества
- Отсутствие ground truth изображений

**Схема:**
1. **Offline:** Hold-out (80/20), 5-fold CV, bootstrap sampling
2. **Online:** A/B тестирование промптов, continuous monitoring
3. **Экспертная:** Еженедельные оценки 50 изображений

**Приоритеты:** Бренд-бук > качество > скорость

### Описание структуры бейзлайна

**Бейзлайн 1:** Прямое использование FLUX.1-dev
- Предобработка: базовый перевод + keywords
- Ожидания: CLIP ~0.65, бренд-бук ~2.5/5

**Бейзлайн 2:** Обогащение через LLM
- Предобработка: анализ категории + корпоративные цвета
- Ожидания: CLIP ~0.75, бренд-бук ~3.5/5

### Стратегии дальнейшего развития

**Краткосрочные:**
- Оптимизация промпт-шаблонов
- A/B тестирование моделей

**Долгосрочные:**
- Fine-tuning на собранных данных
- LoRA адаптация под бренд
- Ensemble моделей

### Анализ работы модели

**Сильные стороны:** Качество 3D-рендера, понимание промптов, стабильность цветов

**Слабые стороны:** Медленная генерация, чувствительность к формулировкам

**EDA результаты:** 75% изображений usable, средний балл 3.89/5

### Риски данного этапа

1. **GPU память overflow** - ограничение до 2048x2048
2. **Несоответствие бренд-буку** - шаблоны промптов + экспертная валидация
3. **Недостаток данных** - активный сбор в пилоте

### Необходимый результат этапа

- Определенные метрики и схема валидации
- Прототипы бейзлайнов с baseline метриками
- План развития модели
- Риски и стратегии снижения
